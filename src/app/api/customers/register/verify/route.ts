...
export async function POST(request: NextRequest) {
  try {
    ...
    const actualHash = hashOtpCode(code, email, 'register');
    ...
    // 1 yıllık session cookie oluştur (kayıt sonrası otomatik oturum açma)
    ...
    console.error('Error verifying customer register OTP:', error);

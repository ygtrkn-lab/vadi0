...
export default function ImageUpload({ value, onChange, label, required, isDark = false }: ImageUploadProps) {
  ...
  const handleFileUpload = async (file: File) => {
    ...
    if (!validTypes.includes(file.type)) {
      setError('Geçersiz dosya tipi. Sadece JPEG, PNG, GIF ve WebP desteklenir.');
      return;
    }
      ...
      {value ? (
        ...
        >
          {uploading ? (
            ...
            <div className="flex flex-col items-center py-4">
              <svg className={`w-10 h-10 mb-3 ${isDark ? 'text-neutral-600' : 'text-slate-400'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
